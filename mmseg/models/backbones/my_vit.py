...
class MYMultiheadAttention(BaseModule):
    ...
    def __init__(self,
        ...
        #                                   **kwargs)
        self.attn = ViTSelfAttention(embed_dims, num_heads, attn_drop,
                                    **kwargs)
    ...
    def forward(self,
        ...
        **kwargs allow passing a more general data flow when combining
        ...
        Args:
            ...
            hidden_states (Tensor): Instead of qkv.
...
class ViTSelfAttention(BaseModule):
    def __init__(self, 
                 embed_dims, 
                 num_heads, 
                 attn_drop, 
        ...
        self.embed_dims = embed_dims
        assert self.head_dim * num_heads == self.embed_dims, "embed_dims must be divisible by num_heads"
        ...
        self.in_proj = nn.Linear(self.embed_dims, 3*self.embed_dims, bias=bias)
        self.out_proj = NonDynamicallyQuantizableLinear(self.embed_dims, self.embed_dims, bias=bias)

    def transpose_for_scores(self, x: torch.Tensor) -> torch.Tensor:
        new_x_shape = x.size()[:-1] + (int(self.num_heads), self.head_dim)
    ...
    def forward(
        ...
        batch_size, sub_seq_length, embed_dim = hidden_states.size()
        # [batch_size, num_heads, sub_seq_len, head_size]
        qkv = self.in_proj(hidden_states).view(batch_size, sub_seq_length, 3, 
                            self.num_heads, self.head_dim).permute(2, 0, 3, 1, 4)
        # make torchscript happy (cannot use tensor as tuple)
        query, key, value = qkv[0], qkv[1], qkv[2]
        # [batch_size, num_heads, sub_seq_len, head_size]
        
        output_size = (query.size(0), query.size(1), query.size(2), key.size(2))
    
        # [batch_size, num_heads, sub_seq_len, head_size] -> [batch_size * num_heads, sub_seq_len, head_size]
        query = query.view(output_size[0]*output_size[1], output_size[2], -1)
        key = key.view(output_size[0]*output_size[1], key.size(2), -1)
        value = value.view(output_size[0]*output_size[1], value.size(2), -1)
        ...
        # output_size = (value.size(0), value.size(1), query.size(2), 
        #                value.size(3))
        ...
        # new_attn_output_shape = attn_output.size()[:-2] + (int(self.embed_dims),)
        # # outputs = attn_output.view(*new_attn_output_shape)
        # outputs = self.out_proj(attn_output.view(*new_attn_output_shape))
        # return outputs
        attn_output = attn_output.transpose(0, 1).contiguous().view(sub_seq_length * batch_size, embed_dim)
        attn_output = self.out_proj(attn_output)
        attn_output = attn_output.view(batch_size, sub_seq_length, embed_dim)
        return attn_output
...
class MYTransformerEncoderLayer(BaseModule):
    ...
    def build_attn(self, attn_cfg):
        ...
        self.attn = MYMultiheadAttention(**attn_cfg)
